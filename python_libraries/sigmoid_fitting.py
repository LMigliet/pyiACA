...
def Four_Parameter_Sigmoid(x, a, b, c, d):
...
def sigmoid_fitting_parallel(df, NMETA, func, p0, bounds, maxfev, core_number):
    ...
    if func == Five_Parameter_Sigmoid:
        ...
        NPARAM = 6
        df_cols = ['a', 'b', 'c', 'd', 'e', 'f']
    elif func == Four_Parameter_Sigmoid:
        NPARAM = 4
        df_cols = ['a', 'b', 'c', 'd']
...
def pivot_fitting(df, func, NMETA, bounds, maxfev, core_number, thresh_MSE, sample_number=50):
    """
    ...
    if func == Five_Parameter_Sigmoid:
        ...
        initial_p0 = (0, 0, 0, 0, 0, 0)
    elif func == Four_Parameter_Sigmoid:
        initial_p0 = (0, 0, 0, 0)
    
    df_fitted_initial, _ = sigmoid_fitting_parallel(df.sample(sample_number), NMETA,
                                                    func, initial_p0, 
...
def fit_by_target(df, func, NMETA, bounds, maxfev, core_number, thresh_MSE):
    ...
    for target, df_ in df.groupby('Target'):
        ...
        p0_initial = pivot_fitting(df_, func,
                              NMETA, bounds, maxfev, core_number, thresh_MSE)
        
        print(f'Target: {target}, p0: {p0_initial}')
        
        df_fitted_param, df_fitted_curve = sigmoid_fitting_parallel(df_, 
                                                                    NMETA, func, p0_initial, 
                                                                    bounds, maxfev, core_number)
        
        fitted_param.append(df_fitted_param)
        fitted_curve.append(df_fitted_curve)
        
    return pd.concat(fitted_param, axis=0).reset_index(drop=True), pd.concat(fitted_curve, axis=0).reset_index(drop=True)


def fit_by_assay(df, func, NMETA, bounds, maxfev, core_number, thresh_MSE):
    
    fitted_param = []
    
    fitted_curve = []
    
    for assay, df_ in df.groupby('Assay'):
        ...
        print(f'Assay: {assay}, p0: {p0_initial}')
